...
const Task = ({ title, date, icon, color, id, isChecked, priority, color2, size, name, color3 }) => {
        ...
        {/* </div> */}
          ...
          <DeleteForeverIcon className="text-red-400 hover:scale-125 transition-all duration-150 ease-out" fontSize="small" /> 
            ...
            <div className="cursor-pointer pb-[2px]" >
                 ...
                 {date && (
                  <div className="flex items-center justify-center px-1 py-1 rounded space-x-1 bg-gray-200 dark:bg-slate-900">
                  <DateRangeOutlinedIcon style={{fontSize: "15px"}} className="text-blue-400"/>
                   <h1 className='text-blue-500 text-sm font-semibold md:text-xs'>{date}</h1>
                  </div>
                 )}
                  ...
                  {name == "Complete" ? (
                    ...
                    <div className={`flex items-center justify-center rounded px-1 [0.5px] ${priority == "green" ? 'bg-green-200 text-green-700' : priority == "blue" ? 'bg-blue-200 text-blue-700' : priority == "yellow" ? 'bg-yellow-500 text-gray-900' : priority == "red" ? 'bg-red-200 text-red-600 animate-pulse' : "bg-gray-200 text-gray-600"}`}>
                  ...
                  <div class="w-full h-2 mb-4 bg-blue-100 rounded-full relative top-3">

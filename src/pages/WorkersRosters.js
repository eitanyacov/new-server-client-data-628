...
import { Dialog, DialogContent, TextField, DialogActions, DialogTitle, Alert } from '@mui/material'
...
const WorkersRosters = () => {
    ...
    const getData = () => {
      const id = result?.subId
      return axios.get(`https://nartina.com/api/user/get-user-rosters/${id}/${page}/10/date`)
    }
    ...
    const {data, refetch} = useQuery('rosters-map', ()=> getData(),
      ...
      const getDates = (e) => {
        e.preventDefault();
        axios.get("https://nartina.com/api/user/rosters-between/" + result?.subId + "/startDate/0/150/" + date1 + "/" + date2)
      ...
      const getDates2 = (e) => {
        e.preventDefault();
        axios.get("https://nartina.com/api/user/workers-rosters-2/" + result?.subId + "/" + date1 + "/" + date2 + "/" + date3 + "/" + date4 + "/" + date5 + "/" + date6 + "/" + date7)
    ...
    const getWorkers = () => {
      ...
      const addRoster = (e) => {
        ...
        //   return
        // }
          axios.post("https://nartina.com/api/user/add-roster/" + result?.subId + "/" + workerId , {
          startDate,
          endDate,
          name: workerFullName,
          ...
          <Alert severity="error">יש לעשות יציאה ולהרשם שוב מטעמי בטיחות  
            ...
            <form onSubmit={addRoster}>
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                  ...
                  <div>
                    <label for="phoneNumber" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white text-right">התחלת משמרת</label>

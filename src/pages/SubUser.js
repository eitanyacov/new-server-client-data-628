...
const SubUser = () => {
    ...
    const [address, setAddress] = useState("worker-role");
    // const [companyName, setCompanyName] = useState("");
    const [phoneNumber, setPhoneNumber] = useState();
    const [dealerlicensed, setDealerlicensed] = useState("worker-role");
    const [password, setPassword] = useState("");
    const [subRole, setSubRole] = useState("");
    const [confirmPassword, setConfirmPassword] = useState("");
    const [city, setCity] = useState("worker-role");
    const [addressNumber, setAddressNumber] = useState("worker-role"); 
    ...
    const roles = ["WORKER", "MANGER", "MASTER"]
  ...
  return (
<div className={`${globalTheme != "light" && "bg-gray-800 dark"} `}>
{flag ? (
  <form onSubmit={postData} className={`${!hebrew ? 'airx:mr-[265px] ml-2' : 'airx:ml-64'} mt-20 px-6 pt-4 pb-12 rounded-lg`}> 
  <h1 class="text-xl font-bold leading-tight tracking-tight md:text-2xl text-[#333] dark:text-white relative bottom-1 dark:bottom-0">
  Add new user to your account
    ...
    <div class="grid md:grid-cols-2 md:gap-6">
      {/* <div class="relative z-0 w-full mb-6 group">
          ...
          <label for="floating_phone" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6" onChange={(e)=> setPhoneNumber(e.target.value)}>Phone number (123-456-7890)</label>
      </div> */}
      <div class="relative z-0 w-full mb-6 group">
          <input type="text" value={phoneNumber} class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required onChange={(e)=> setPhoneNumber(e.target.value)}/>
      ...
      <div class="relative z-0 w-full mb-6 group">
        ...
        <select onChange={(e) => setSubRole(e.target.value)} className="bg-gray-50 h-[42px] relative top-1 border border-gray-300 text-gray-500 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-[#ccc] dark:focus:ring-blue-500 dark:focus:border-blue-500">
            <option selected="">Select Role</option>
            {roles.map(r => (
            <option value={r}>{r}</option>
            ))}
        </select>
    <button type="submit" class="text-white bg-blue-700 relative top-7 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Submit</button>
  </form>
) : (
  <section class={`bg-white dark:bg-gray-900 fixed h-screen ${windowHeight < 620 ? 'pt-1' : windowHeight < 650 ? 'pt-4' : windowHeight < 680 ? 'pt-8' : windowHeight < 710 ? 'pt-10' : windowHeight < 750 ? 'pt-14' : windowHeight < 780 ? 'pt-20' : windowHeight < 850 ? 'pt-24' : 'pt-28'} ${hebrew ? 'airx:ml-64' : 'airx:mr-64'}`}>
    <div class="py-8 px-4 mx-auto max-w-screen-xl text-center lg:py-16 lg:px-12">
        <a href="#" class="inline-flex justify-between items-center relative top-2 py-1 px-1 pr-4 mb-7 text-sm text-gray-700 bg-gray-100 rounded-full dark:bg-gray-800 dark:text-white hover:bg-gray-200 dark:hover:bg-gray-700" role="alert">
            <span class="text-xs bg-blue-600 rounded-full text-white px-4 py-1.5 mr-3">New</span> <span class="text-sm font-medium">handle all your work in one place.</span> 
            <svg class="ml-2 w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
        </a>
        {/* <h1 class="mb-4 text-4xl font-extrabold tracking-tight leading-none text-gray-900 md:text-5xl lg:text-6xl dark:text-white">We invest in the world’s potential</h1> */}
        <h1 class="mb-4 text-4xl font-extrabold tracking-tight leading-none text-gray-900 md:text-5xl lg:text-6xl dark:text-white">The CRM software that's simple to set up</h1>
        <p class="mb-8 text-lg font-normal text-gray-500 lg:text-xl sm:px-16 xl:px-48 dark:text-gray-400">Here at Nartina we focus on markets where technology, innovation, and capital can unlock long-term value and drive economic growth.</p>
        <div class="flex flex-col mb-8 lg:mb-16 space-y-4 sm:flex-row sm:justify-center sm:space-y-0 sm:space-x-4">
            <div class="inline-flex justify-center items-center cursor-pointer py-3 px-5 text-base font-medium text-center text-white rounded-lg bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 dark:focus:ring-blue-900" onClick={()=> setFlag(!flag)}>
                Add User 
        ...
        <div class="px-4 mx-auto text-center md:max-w-screen-md lg:max-w-screen-lg lg:px-36 relative bottom-5">
...
<Dialog open={openx}>
            ...
            <form onSubmit={postData} >
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="name" value={firstName} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Type product name" required="" onChange={(e)=> setFirstName(e.target.value)}/>
                    </div>
                    <div>
                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Last Name</label>
                        <input type="text" name="name" value={lastName} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Type product name" required="" onChange={(e)=> setLastName(e.target.value)}/>
                    </div>
                  <div>
                      <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">User email</label>
                      <input type="email" name="email" value={email} class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="name@company.com" required="" onChange={(e)=> setEmail(e.target.value)}/>
                  </div>
                    <div>
                        <label for="category" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">User Role</label>
                        <select onChange={(e) => setSubRole(e.target.value)} className="bg-gray-50 relative border border-gray-300 text-gray-500 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-3 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-[#ccc] dark:focus:ring-blue-500 dark:focus:border-blue-500">
                              <option selected="">Select Role</option>
                              {roles.map(r => (
                              <option value={r}>{r}</option>
                              ))}
                    ...
                    <div>
                      ...
                      <input type="password" name="password" value={password} placeholder="••••••••" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required="" onChange={(e)=> setPassword(e.target.value)}/>
                  </div>
                  <div>
                      <label for="confirm-password" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Confirm password</label>
                      <input type="confirm-password" name="confirm-password" value={confirmPassword} placeholder="••••••••" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required="" onChange={(e)=> setConfirmPassword(e.target.value)}/>
                  </div>
 ...
 <Snackbar open={openAlert} autoHideDuration={10000} onClose={handleClose} anchorOrigin={{vertical: 'bottom', horizontal: 'left'}}>
        <Alert
          ...
          sx={{ width: "100%" }}
        >
          {hebrew ? "successfully added new user" : "משתמש הוכנס בהצלחה"}
        </Alert>
      </Snackbar>
      </div>
  )

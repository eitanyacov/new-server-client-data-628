...
const Supplier = () => {
  ...
  const getData = () => {
    const id = result?.id
      return axios.get(`https://nartina.com/api/user/all-invoices-paging-by-supplier/${id}/${page}/50/date/${supplierid}`)
  }
  ...
  const {data, refetch} = useQuery('supplier-invoices', ()=> getData(),
    {
      ...
      // useEffect(()=> {
      //   const id = result?.id
      //   axios.get(`https://nartina.com/api/user/all-invoices-paging-by-supplier/${id}/${page}/1/date/${supplierid}`)
      ...
      // const getInvoicesPaging = () => {
      //   const id = result?.id
      //   axios.get(`https://nartina.com/api/user/all-invoices-paging-by-supplier/${id}/${page}/25/date/${supplierid}`)
      //   .then(res => {console.log(res.data)
      //   setPaging(res.data.content)
      //   setNumber(res.data.totalElements)
      //   setPages(res.data.totalPages)
     ...
     <label className={`flex ${hebrew ? "justify-start" : "justify-end"} text-blue-500 font-mono`}>{hebrew ? "cheque date" : "תאריך צ'ק"}</label>
         ...
         {hebrew ? (
                <TextField InputLabelProps={{
                  shrink: true,
                }} disabled={paymentMethod != "צ'ק"} label="cheque" type='text' className='bg-white focus:outline-none focus:ring focus:border-blue-500 mt-1' defaultValue={invoice?.checkNumber} placeholder="מספר צ'ק" onChange={e => setCheckNumber(e.target.value)}/>
  ...
  <ThemeProvider theme={theme}>
         <TextField InputLabelProps={{
        shrink: true,
      }} disabled={paymentMethod != "צ'ק"} label="מספר צ'ק" type='text' className='bg-white focus:outline-none focus:ring focus:border-blue-500 mt-1' defaultValue={invoice?.checkNumber} placeholder="מספר צ'ק" onChange={e => setCheckNumber(e.target.value)}/>
         </ThemeProvider>
</CacheProvider>
         )}
         {hebrew ? (
  ...
  {hebrew ? (
    <TextField type='text' label='invoive amount' defaultValue={invoice?.amount} className='bg-white focus:outline-none focus:ring focus:border-blue-500 p-2 mt-1' placeholder='סכום חשבונית' onChange={e => setAmount(e.target.value)}/>
...
<Dialog open={openNoteDialog}>
    ...
    <Snackbar open={errorMode} autoHideDuration={20000} onClose={handleClose3} anchorOrigin={{vertical: 'bottom', horizontal: 'left'}}>
      ...
      <Snackbar open={alert} autoHideDuration={30000} onClose={handleCloseAlert} anchorOrigin={{vertical: 'buttom', horizontal: hebrew ? 'right' : 'left'}}>
        ...
        >
          {/* <AlertTitle>שים לב</AlertTitle> */}
          {hebrew ? <>all supplier invoices</> : <>כל החשבוניות של הספק</>}
        </Alert>
      </Snackbar>
        </>

...
const Restaurant = () => {
  ...
  //  const [openRestaurntAlert, setOpenRestaurntAlert] = useState(false)
   ...
   const [totalCibus, setTotalCibus] = useState(0);
   const [totalTenBis, setTotalTenBis] = useState(0);
   const [totalGoodi, setTotalGoodi] = useState(0);
   const [totalMishloha, setTotalMishloha] = useState(0);
   const [totalWallt, setTotalWallt] = useState(0);
   const [totalOther, setTotalOther] = useState(0);
   const [totalOther2, setTotalOther2] = useState(0);
  ...
  // const getData = () => {
  //   const id = result?.id
  //   return axios.get("https://nartina.com/api/user/all-restaurant-reports/" + id)
  // }
  
  // const {data, refetch} = useQuery('restaurants', ()=> getData(),
  //   {
  ...
  const getCust = () => {
    const id = result?.id
    return axios.get(`https://nartina.com/api/user/all-restaurants-paging/${id}/${page}/50/date`)
  }
  
  const {data, refetch} = useQuery('restaurants', ()=> getCust(),
    {
      // enabled: !!supplier?.name,
      // staleTime: 300000
      refetchOnMount: false,
  ...
  const printValues = (e) => {
    e.preventDefault();
      ...
      refetch()
      Swal.fire("!עודכן", '!דו"ח מסעדות עודכן בהצלחה', "success")
...
// }
      ...
      const postDataRestaurntDaily = (e) => {
        ...
        if(dateRest == "") {
          ...
          navigate("/restaurant")
          Swal.fire("!הצלחה", '!דו"ח מסעדות הוכנס בהצלחה', "success")
      ...
      const getDailyIncomesForPdf = () => {
        ...
        .then(res => {setRestaurants(res.data.sortedIncomes)
          setRecords(res.data.numberOfRecords)
          setTotalSum(res.data.totalIncomes)
          setTotalCibus(res.data.totalCibus)
          setTotalGoodi(res.data.totalGoodi)
          setTotalMishloha(res.data.totalMishloha)
          setTotalTenBis(res.data.totalTenBis)
          setTotalWallt(res.data.totalWallt)
          setTotalOther(res.data.totalOther)
          setTotalOther2(res.data.totalOther2)
          setMonth("")
          setYear("")
        console.log(res.data)})     
        .catch(err => console.log(err))
        .finally(setMonth(""), setYear(""))
      }
  ...
  return (
         ...
         <ReactToPrint
          trigger={() => {
            ...
            return <a href="#">
                    ...
                    <div className="flex items-center justify-center space-x-1 bg-blue-700 hover:bg-blue-600 rounded-md px-2 py-[2px] w-fit" onClick={printPdf}> 
                    <h1 className="font-mono text-white font-semibold">הדפס</h1>
    ...
    {totalCibus > 0  && <h1 className='font-mono '><span className='text-sm font-sans'>₪</span> {Number(Math.round(totalCibus)).toLocaleString()} סיבוס</h1>}
    {totalTenBis > 0  && <h1 className='font-mono '><span className='text-sm font-sans'>₪</span> {Number(Math.round(totalTenBis)).toLocaleString()} תן ביס</h1>}
    {totalWallt > 0  && <h1 className='font-mono '><span className='text-sm font-sans'>₪</span> {Number(Math.round(totalWallt)).toLocaleString()} וולט</h1>}
    {totalMishloha > 0  && <h1 className='font-mono '><span className='text-sm font-sans'>₪</span> {Number(Math.round(totalMishloha)).toLocaleString()} מישלוחה</h1>}
    </div>
    <div className='flex items-center justify-end space-x-2 mr-16 relative bottom-2'>
    {totalGoodi > 0  && <h1 className='font-mono '><span className='text-sm font-sans'>₪</span> {Number(Math.round(totalGoodi)).toLocaleString()} גודי</h1>}
    {totalOther > 0  && <h1 className='font-mono '><span className='text-sm font-sans'>₪</span> {Number(Math.round(totalOther)).toLocaleString()} אחר</h1>}
    {totalOther2 > 0  && <h1 className='font-mono '><span className='text-sm font-sans'>₪</span> {Number(Math.round(totalOther2)).toLocaleString()} אחר2</h1>}
    </div>
          </div>
        )}
    </div>
        ...
        <form onSubmit={getExcel} className='flex space-x-4'>
            ...
            {months.map(a => (
                ...
                <MenuItem value={a}>{a}</MenuItem>
                  ...
                  <button type='submit' disabled={(year == "" || month == "") } className='bg-blue-700 px-2 py-1 font-mono text-white hover:bg-blue-300 hover:text-blue-600 font-semibold rounded-lg relative top-2'>הדפס דו"ח מסעדות</button>

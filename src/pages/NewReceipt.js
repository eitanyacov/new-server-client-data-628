...
const NewReceipt = () => {
    ...
    const arr2 = ["מזומן", "צ'ק", "הוראת קבע", "כרטיס אשראי", "העברה בנקאית", "ביט", "אחר"];

    // const www = [
    //   {id: 1, description: "yyyy" , price: 44},
    //   {id: 2, description: "eeee" , price: 555},
    //   {id: 3, description: "ttt" , price: 432},
    //   {id: 4, description: "wweee" , price: 123},
    //   {id: 5, description: "uuu" , price: 876}
    // ]
    ...
    const getCust = () => {
      ...
      const {data, refetch} = useQuery('customers', ()=> getCust(),
        ...
        const addReceipt = (e) => {
          if(paidOrNo == "?האם שולם") {
            alert("יש לציין האם שולם")
            return
          }
          if(paymentMethod == "צורת תשלום") {
            alert("יש לציין צורת תשלום")
            return;
          }
            e.preventDefault();
            axios.post("https://nartina.com/api/user/add-receipt-to-user/" + result?.id + "/" + customerId , {
            paidOrNo,
            date,
            checkDate,
            checkNumber,
        ...
        const deleteReceipt = () => {
            axios.delete("https://nartina.com/api/user/delete-receipt/" + id, {
              headers: {
                Authorization: 'Bearer ' + result?.token,
        ...
        const addAmount = (sum) => {
            axios.get("https://nartina.com/api/user/add-amount-receipt/" + id + "/" + sum)
            .then(res => console.log(res.data))
            .catch(err => console.log(err.response.data))
          }
        ...
        const addItemsToReceipt = (e) => {
            e.preventDefault();
            axios.post("https://nartina.com/api/file/add-item-to-receipt/" + id , {
              description,
              unit,
              unitPrice,
              // total: total / 1.17
        ...
        const handleChange = (e) => {
          ...
          const deleteItem = (itemId) => {
            items.map(i => {
              if(i.id == itemId) {
                setSum(sum - i.total)
                axios.delete("https://nartina.com/api/user/delete-receipt-item/" + itemId + "/" + id + "/" + i.total, {
                  headers: {
                    Authorization: 'Bearer ' + result?.token,
        ...
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <form onSubmit={addReceipt}>
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div>
                        <label for="brand" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white text-right">?האם שולם</label>
                        <select id="category" onChange={handleChange2} class="bg-gray-50 h-[40px] border pr-2 border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-3 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            <option selected="" className='text-right'>?האם שולם</option>
                        {arr.map(pay => (
                                <option value={pay} className='text-right'>{pay}</option>
                            ))}
                        </select>
                    </div>
                    <div>
                        <label for="category" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white text-right">בחר לקוח</label>
                        <select id="category" onChange={handleChange} class="bg-gray-50 h-[40px] border pr-2 border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                    ...
                    <div>
                        <label for="category" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white text-right">צורת תשלום</label>
                        <select id="category" disabled={paidOrNo != "שולם"} onChange={handleChange3} class="bg-gray-50 h-[40px] border pr-2 border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            <option selected="" className='text-right'>צורת תשלום</option>
                            {arr2.map(pay => (
                                <option value={pay} className='text-right'>{pay}</option>
                            ))}
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 h-[40px] min-w-max focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required={true} onChange={e => setDate(e.target.value)} />
                    </div>
                    {paymentMethod == "צ'ק" && (
                  <>
                   <div>
                 ...
                 <div>
                   <div className='flex justify-end items-center space-x-1'>
                     ...
                     <input disabled={paymentMethod != "צ'ק"} type="date" name="name" value={checkDate} class="bg-gray-50 h-[41px] min-w-max w-full text-right border placeholder:text-right border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="תאריך צ'ק" required="" onChange={(e)=> setCheckDate(e.target.value)}/>
                 </div>
                  </>
                 )}
                ...
                {/* <div className='flex justify-between mt-8'>
                        <div className='flex space-x-7'>
                            ...
                            <h1 className='text-gray-800 tracking-wide font-bold'>סכום</h1>
                            <h1 className='text-gray-500 text-xs tracking-wide font-medium'>(כולל מע"מ)</h1>
                            </div>
                            <div>
                            <h1 className='text-gray-800 tracking-wide font-bold'>מחיר יחידה</h1>
                            <h1 className='text-gray-500 text-xs tracking-wide font-medium relative left-[7px]'>(כולל מע"מ)</h1>
                            </div>
                            <h1 className='text-gray-800 tracking-wide font-bold'>יחידות</h1>
                        </div>
                        <h1 className='text-gray-800 tracking-wide font-bold'>תיאור</h1>
                    </div> */}
                    {/* <div className='flex justify-between mt-8'>
                        <div className='flex space-x-8 ml-4'>
                            <h1 className='text-gray-800 tracking-wide font-bold'>{total}</h1>
                            <h1 className='text-gray-800 tracking-wide font-bold'>{unitPrice}</h1>
                            <h1 className='text-gray-800 tracking-wide font-bold'>{unit}</h1>
                        </div>
                        <h1 className='text-gray-800 tracking-wide font-bold'>{description}</h1>
                    </div> */}
                <div class="sm:col-span-2 mt-4">
                    <hr />
                </div>
                <div className='flex justify-between mt-5'>
                <button type="submit" disabled={date == "" || paidOrNo == "" || customerId == ""} class="text-white tracking-wide inline-flex items-center bg-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:outline-none focus:ring-indigo-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-800">
                    הכנס חשבונית

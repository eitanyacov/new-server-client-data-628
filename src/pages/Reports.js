...
const Reports = () => {
  ...
  const { hebrew, reload, setReload } = useContext(ThemeContext) 

  const deffrrealPayments = JSON.parse(localStorage.getItem('deffrreal'));
  ...
  // useEffect(()=> {
  //   const id = result?.id;
  //   axios.get(`https://nartina.com/api/user/deferralPayments-total-amount-no-limit/${id}`)
  ...
  //   }) 
    ...
    // const getDataAmountInvoices = () => { 
    //   const id = result?.id
    //   return axios.get(`https://nartina.com/api/user/deferral-invoices-amount-no-limit/${id}`)
    // }
    
    // const {data: invoicesTotalMoneyOut} = useQuery('defrreal-payments-total-amount-invoices', ()=> getDataAmountInvoices(),
    //   {
    //     // enabled: !!supplier?.name,
    //     // staleTime: 300000
    //     refetchOnMount: deffrrealPayments,
  ...
  const getData = () => {
    const id = result?.id
    return axios.get(`https://nartina.com/api/user/deferralPayments/${id}`)
  }
  
  const {data, refetch} = useQuery('defrreal-payments', ()=> getData(),
    {
      // enabled: !!supplier?.name,
      // staleTime: 300000
      refetchOnMount: deffrrealPayments || reload,
      refetchOnWindowFocus:false
...
useEffect(()=> {
  ...
  // const getSchdulers = () => {
  //   const id = result?.id;
  //   axios.get(`https://nartina.com/api/user/deferralPayments/${id}`)
  ...
  const commitChanges = ({ added, deleted, changed }) => {
    ...
    if(changed) {
      ...
      schedulers.map(s => {
        if(changed[s.id]) {
          ...
          axios.post("https://nartina.com/api/user/update-deferral-payments/" + s?.id, {
    ...
    if(deleted) {
      axios.delete("https://nartina.com/api/user/delete-deferralPayment/" + deleted,  {
        headers: {
          Authorization: 'Bearer ' + result?.token,
  ...
  return (
    <>
      {/* <div className={`mt-14 w-[98%] md:[78%] lg:w-[98%] ${hebrew && "airx:ml-[183px]"} airx:w-[82%] xlu:w-[80.5%] uuu:w-[81%] uux:w-[81.5%] 2xl:w-[80%] pxl:w-[81.5%] mt-2 min-h-screen overflow-hidden`}> */}
      ...
      {/* <Snackbar open={totalAmount} autoHideDuration={15000} onClose={handleCloseAmount} anchorOrigin={{vertical: 'bottom', horizontal: hebrew ? 'right' : 'left'}}>
        <Alert
          ...
          sx={{ width: "100%" }}
        >
          {hebrew ? <h1>total Deferred payments<span className='text-red-700 font-semibold'> {Number(isFinite(allTotalMoneyOut?.data) ? allTotalMoneyOut?.data : 0).toLocaleString()}</span> NIS
          </h1> :
          <h1>סה"כ תשלומים דחויים<span className='text-red-700 font-semibold'> {Number(isFinite(allTotalMoneyOut?.data) ? allTotalMoneyOut?.data : 0).toLocaleString()}</span> ש"ח
          </h1>}
        </Alert>
      </Snackbar> */}

      {/* <Snackbar open={totalAmountInvoices} autoHideDuration={15000} onClose={handleCloseAmount2} anchorOrigin={{vertical: 'bottom', horizontal: hebrew ? 'left' : 'right'}}>
        <Alert
          onClose={handleCloseAmount2}
          severity="error"
          sx={{ width: "100%" }}
        >
         {hebrew ?  <h1>Deferred suppliers payments<span className='text-red-700 font-semibold'> {Number(isFinite(invoicesTotalMoneyOut?.data) ? invoicesTotalMoneyOut?.data : 0).toLocaleString()}</span> NIS
          </h1> :
           <h1>סה"כ צ'קים ספקים דחויים<span className='text-red-700 font-semibold'> {Number(isFinite(invoicesTotalMoneyOut?.data) ? invoicesTotalMoneyOut?.data : 0).toLocaleString()}</span> ש"ח
           </h1>}
        </Alert>
      </Snackbar> */}

      <Snackbar open={errorMode} autoHideDuration={20000} onClose={handleClose3} anchorOrigin={{vertical: 'bottom', horizontal: hebrew ? 'right' : 'left'}}>
        <Alert

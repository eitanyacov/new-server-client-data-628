...
const CurrentWaresMonthInvoices = () => {
    ...
    const getData = () => {
      const id = result?.id
      return axios.get('https://nartina.com/api/user/total-monthly-invoices-wares/' + id + "/" + currentMonth + "/" + currentYear)
    }
    
    const {data, refetch} = useQuery('current-month-wares-invoices', ()=> getData(),
      {
    ...
    useEffect(() => {
      ...
      const getInvoices = () => {
        axios.get('https://nartina.com/api/user/total-monthly-invoices-wares/' + user?.id + "/" + currentMonth + "/" + currentYear)
  ...
  return (
      ...
      {hebrew ? (
       ...
       <div class="bg-white dark:bg-gray-800 shadow-md sm:rounded-lg">
           <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
               ...
               <button type="button" class="flex items-center justify-center text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800" onClick={()=> navigate('/waybills-month')}>
                         ת. משלוח
                   </button>
               ...
               <div className='hidden md:flex items-center justify-center space-x-2'>
                  ...
                  <h1 className='hidden md:inline font-mono font-semibold text-xl text-[#495057] dark:text-[#ccc] relative bottom-[1px]'>חשבוניות סחורה</h1>
               </div>
               ...
               <h1 className='inline md:hidden text-center font-mono font-semibold text-lg text-[#333333] dark:text-[#ccc]'>קניית סחורה</h1>
    ...
    <Snackbar open={openAlert} autoHideDuration={10000} onClose={handleClose} anchorOrigin={{vertical: 'buttom', horizontal: 'left'}}>
        <Alert
          onClose={handleClose}
          severity="success"
          sx={{ width: "100%" }}
      ...
      <Snackbar open={openAlertAdd} autoHideDuration={10000} onClose={handleClose} anchorOrigin={{vertical: 'buttom', horizontal: 'left'}}>
        <Alert
     ...
     <Dialog open={openForm}>
      <div className='flex items-center justify-end p-2'>
        ...
        <Select disabled={paidOrNo == "לא שולם" || refundInvice} onChange={handleChange2} className='bg-white w-52 focus:outline-none text-right focus:ring focus:border-blue-500 mt-1' defaultValue={invoice?.paymentMethod}>
     ...
     <label className={`flex ${hebrew ? "justify-start" : "justify-end"} text-blue-500 font-mono`}>{hebrew ? "cheque date" : "תאריך צ'ק"}</label>
        ...
        {hebrew ? (
         ...
         <button type='submit' className='bg-blue-200 px-2 py-2 font-mono text-blue-600 hover:bg-blue-300 hover:text-blue-600 font-semibold rounded-lg'>{hebrew ? "update" : "עדכן חשבונית"}</button>
                ...
                {hebrew ? (
                  <>
                <TextField type='text' label='supplier name' value={invoice?.supplierName} className='bg-white text-right focus:outline-none focus:ring focus:border-blue-500 p-2 mt-1'/>
                ...
                <ThemeProvider theme={theme}>
                <div dir="rtl">
                <TextField type='text' label='שם ספק' value={invoice?.supplierName} className='bg-white text-right focus:outline-none focus:ring focus:border-blue-500 p-2 mt-1'/>
                </div>
                </ThemeProvider>
                </CacheProvider>
                  </>
...
<InputLabel id="demo-simple-select-label" className={`${hebrew ? "text-left" : "text-right"}`}>{hebrew ? "is paid?" : "?האם שולם"}</InputLabel>
        <Select disabled={refundInvice} onChange={handleChange1} className='bg-white focus:outline-none focus:ring focus:border-blue-500 mt-1 text-right' defaultValue={invoice?.paidOrNo}>
...
export default CurrentWaresMonthInvoices